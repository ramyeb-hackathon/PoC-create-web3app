...
import { downloadAndExtractRepo, validateProjectName, getPkgManager, } from "./helpers/helpers.js";
...
function processRecursivlyFilesInDeepFirstSearchPostOrder(_depth, _fileOrDir, projectName) {
    if (fs.lstatSync(_fileOrDir).isFile()) {
        ...
        fs.readdir(_fileOrDir, (_err, filesOrDirs) => {
            filesOrDirs.forEach((element) => {
                processRecursivlyFilesInDeepFirstSearchPostOrder(depth, path.join(_fileOrDir, element), projectName);
            });
        });
    }
}
async function main() {
    ...
    await processRecursivlyFilesInDeepFirstSearchPostOrder(0, process.cwd(), projectName);
